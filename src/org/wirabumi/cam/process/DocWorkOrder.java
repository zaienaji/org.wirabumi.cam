...
public class DocWorkOrder extends AcctServer {
  ...
  public boolean loadDocumentDetails(FieldProvider[] data, ConnectionProvider conn) {
    /*
     * dipakai untuk mengisi DocumentType = AcctServer.DOCTYPE_MatInventory; C_Currency_ID =
     * NO_CURRENCY; DateDoc = data[0].getField("MovementDate"); loadDocumentType(); // lines require
     * doc type Contained Objects: p_lines = loadLines(conn); setelah itu return true;
     */
    DocumentType = "CAM_WO";
  ...
  public Fact createFact(AcctSchema as, ConnectionProvider conn, Connection con,
    ...
    if (!strClassname.equals("")) {
      try {
        ...
        log4jDocWorkOrder
            .error("Error while creating new instance for DocInventoryTemplate - " + e);
      }
    }
    ...
    for (int i = 0; i < p_lines.length; i++) {
      ...
      OBCriteria<AssetAccounts> assetAccountingC = OBDal.getInstance()
          .createCriteria(AssetAccounts.class);
      assetAccountingC.add(Restrictions.eq(AssetAccounts.PROPERTY_ASSET, asset));
      assetAccountingC
          .add(Restrictions.eq(AssetAccounts.PROPERTY_ACCOUNTINGSCHEMA, accountingSchema));
      ...
      if (woA.isDisposed()) {
        ...
        dr = fact.createLine(line, akunDisposalLoss, costCurrency.getId(), costs,
        ...
        cr = fact.createLine(line, akunAkumulasiDepresiasi, costCurrency.getId(),
            (b_Costs.negate()).toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
            conn);
        if (cr == null) {
          continue;
        }
        ...
        // dapatkan total akumulasi depresiasi dengan menjumlahkan semua akun akumulasi depresiasi
        // dari fact acct
        ...
        log4jDocWorkOrder.debug("get accumulated depreciation for asset id : " + asset.getId()
            + " accumulated depreciation account id: "
            + acAkumulasiDepresiasi.getAccount().getId());

        // pindahkan harga perolehan dan akumulasi depresiasi dari cost center lama ke cost center
        // baru
        ...
        // ayat jurnal ke-1.
        log4jDocWorkOrder
            .debug("CreateFact - harga perolehan di cost center baru DR: " + hargaperolehan);
        Account akunHargaPerolehan = new Account(conn, acFixedAsset.getId());
        dr = fact.createLine(lineCostcenterBaru, akunHargaPerolehan, costCurrency.getId(),
            hargaperolehan.toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
        if (dr == null) {
          continue;
        }

        // ayat jurnal ke-2.
        log4jDocWorkOrder.debug(
            "CreateFact - akumulasi depresiasi di cost center baru DR: " + akumulasidepresiasi);
        Account akunAkumulasiDepresiasi = new Account(conn, acAkumulasiDepresiasi.getId());
        cr = fact.createLine(lineCostcenterBaru, akunAkumulasiDepresiasi, costCurrency.getId(),
            (totalAkumulasiDepresiasi.negate()).toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo),
            DocumentType, conn);
        if (cr == null) {
          continue;
        }
        ...
        // ayat jurnal ke-3.
        cr = fact.createLine(lineCostcenterLama, akunAkumulasiDepresiasi, costCurrency.getId(),
            totalAkumulasiDepresiasi.toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
            conn);
        if (cr == null) {
          continue;
        }
        ...
        // ayat jurnal ke-4.
        dr = fact.createLine(lineCostcenterLama, akunHargaPerolehan, costCurrency.getId(),
            hargaperolehan.negate().toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
            conn);
        if (dr == null) {
          continue;
        }
